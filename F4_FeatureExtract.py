...
#
    ...
    for file in files:
        json_path1 = os.path.join(json_path ,file)
        wsi_path1 = wsi_path+ '/' + file.split('.json')[0] + '.svs'

        print(file.split('.json')[0], "begin dong!!")
        sample_name = os.path.basename(wsi_path1).split('.')[0]
        with open(json_path1) as fp:
            print(f"{'Loading json':*^30s}")
            nucleusInfo = json.load(fp)

        globalgraph, edge_info = constructGraphFromDict(wsi_path1, nucleusInfo, distanceThreshold, k, level)
        ...
        output_path1 = os.path.join(output_path, sample_name)
        try:
            os.makedirs(output_path1)
        except:
            pass
        for i in col_dist.keys():
            vertex_csvfile = os.path.join(output_path1, sample_name + '_Feats_' + i + '.csv')
